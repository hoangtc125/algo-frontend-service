...
import { Layout, Menu, FloatButton, Image, Modal, Result, Affix } from 'antd';
...
const MainLayout = () => {
    ...
    useEffect(() => {
        const scrollToTop = () => {
            window.scrollTo({
                top: 0,
                behavior: 'smooth' // Thêm 'smooth' nếu bạn muốn có hiệu ứng cuộn mượt
            });
        };

        // Thực hiện cuộn về đầu trang khi component được render
        if (location.pathname.includes("club/") && location.pathname.includes("events/") && location.hash) {
            return
        }
        scrollToTop();
    }, [location]);
            ...
            <Layout>
                {
                    account &&
                    <Affix>
                        <Sider collapsed={true} theme='light' className='h-screen w-[50px] sm:w-[200px]'>
